...
function get_area($parent = 0)
{
    $areasql="SELECT shipping_area_id as region_id,shipping_area_name as region_name FROM " . $GLOBALS['ecs']->table('shipping_area')." WHERE shipping_area_id IN (SELECT shipping_area_id FROM " . $GLOBALS['ecs']->table('area_region')." WHERE region_id=$parent)";
	return $GLOBALS['db']->getAll($areasql);
}
function get_area_name($id = 0)
{
    $areasql="SELECT shipping_area_name  FROM " . $GLOBALS['ecs']->table('shipping_area')." WHERE  shipping_area_id=$id";
	return $GLOBALS['db']->getone($areasql);
}
/**
