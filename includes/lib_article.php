 ...
 * @return  array
 */
function get_cat_articles($cat_id, $page = 1, $size = 20 ,$requirement='',$user_id='')
{
    //取出所有非0的文章
    if ($cat_id == '-1')
    {
    ...
    if ($res)
    {
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
			...
			$arr[$article_id]['user_id']       = $row['user_id'];
			$arr[$article_id]['hot_num']       = $row['hot_num'];
