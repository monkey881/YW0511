...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, credit_line, last_login, is_validated,userpic FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
...
/*-------------------------------------------------------------------------------------------------------------2013.3.6修改待评论功能---------*/	
	$sql_qaz = "SELECT count(*) FROM `ecs_order_info` WHERE user_id= '$user_id' and is_comment  <> 1";
    $info['is_comment_count'] = $GLOBALS['db']->getOne($sql_qaz);
