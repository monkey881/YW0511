...
function available_shipping_list($region_id_list)
{
    /*$sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s LEFT JOIN ' .
                $GLOBALS['ecs']->table('shipping_area') . ' AS a  ON a.shipping_id = s.shipping_id LEFT JOIN  ' .
                $GLOBALS['ecs']->table('area_region') . ' AS r ON r.shipping_area_id = a.shipping_area_id ' .
            'WHERE r.region_id ' . db_create_in($region_id_list) .
            '   AND s.enabled = 1 ORDER BY s.shipping_order';*/
		
		$sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
                's.shipping_desc, s.insure, s.support_cod ' .
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s ' .
            'WHERE  s.enabled = 1 ORDER BY s.shipping_order';
 ...
 * @return  bool
 */
function update_order_taocan($order_id, $taocan)
{
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_taocan'),
        $taocan, 'UPDATE', "order_id = '$order_id'");
}
/**
 * 得到新订单号
 * @return  string
