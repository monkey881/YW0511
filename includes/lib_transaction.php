...
function edit_profile($profile)
{
    ...
    /* 过滤非法的键值 */
    $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone','userpic');
...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT user_name, birthday, sex, userpic,question, answer, rank_points, pay_points,user_money, user_rank,".
 ...
 * @return  array
 */
function get_basket_list($ps_id)
{
    $sql= "SELECT b.* ,g.goods_name,g.goods_weight 
		 FROM " . $GLOBALS['ecs']->table('user_basket') . " b  LEFT JOIN ". $GLOBALS['ecs']->table('goods') ." g ON b.goods_id=g.goods_id where ps_id=$ps_id ORDER BY b.goods_number DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$z_weigth=$z_weigth+$row['weigth'];
		$basket_list[]=$row;
	}
    return array($z_weigth,$basket_list);
}
/**
 * 取得套餐订单
 * @param   int     $user_id    用户编号
 * @return  array
 */
function get_order_taocan_one($tid)
{
    $sql= "SELECT * 
		 FROM " . $GLOBALS['ecs']->table('order_taocan') . " where tid=$tid";
 ...
 * @return  array
 */
function get_goods_taocan_list($goods_taocan_id,$num=36,$start=0,$brand_id=0)
{
	if($goods_taocan_id==0)
	{
		return;
	}
	else
	{
			...
			$wheresql=" t.goods_taocan_id=$goods_taocan_id ";
	}
	if($brand_id>0)
	{
		$wheresql.=" AND g.brand_id=$brand_id";
	}
     $sql = "SELECT g.goods_id, g.goods_name,g.goods_thumb,g.shop_price,g.goods_number,g.goods_weight,t.num 
		 FROM " . $GLOBALS['ecs']->table('goods') . " g  LEFT JOIN ". $GLOBALS['ecs']->table('taocan_shop') ." t ON g.goods_id=t.goods_id where $wheresql ORDER BY t.num DESC";
	$res = $GLOBALS['db']->query($sql);
    $taocan_list = array();
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		else
		{
			...
			$row['goods_weight4']=$row['goods_weight']*2*4;
			$row['goods_weight5']=$row['goods_weight']*2*5;
			$row['goods_weight6']=$row['goods_weight']*2*6;
			$row['goods_weight8']=$row['goods_weight']*2*8;
			$row['goods_weight10']=$row['goods_weight']*2*10;
		$taocan_list[]=$row;
	}
    return $taocan_list;
}
...
function get_user_orders($user_id, $num = 10, $start = 0,$status='')
{
    ...
    $arr    = array();
	if($status=='dpj'){
		$wheresql=" AND is_comment=0";
	}
    $sql = "SELECT oi.order_id, oi.order_sn, oi.order_status, oi.shipping_status, oi.pay_status, oi.add_time, oi.is_comment,og.goods_id," .
           ...
           " WHERE user_id = '$user_id' $wheresql ORDER BY add_time DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                @$row['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$GLOBALS['_LANG']['received']."</a> ".$comment;
            }
            elseif ($row['shipping_status'] == SS_RECEIVED)
            {
                @$row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span> '.$comment;
            }
            else
            {
                if ($row['pay_status'] == PS_UNPAYED)
                {
                    @$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a> '.$comment;
                }
                else
                {
                    @$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a> '.$comment;
                }
        ...
        {
            $row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['os'][$row['order_status']] .'</span> '.$comment;
        }
...
function get_user_orders_taocan($user_id=0, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();
	$wheresql="";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
	   if($row['status']==2)
	   {
		   $row['handler'] = "<a href=\"user.php?act=rili&tid=".$row['tid']. "\">设置套餐</a> | <a href=\"user.php?act=stop_taocan&tid=" .$row['tid']. "\" onclick=\"if (!confirm('你确认要暂停配送吗？想要开始的时候记得来套餐管理里点击“开始配送”！')) return false;\">暂停配送</a>";
	   }
	    else if($row['status']==3)
	   {
		   $row['handler'] = "<a href=\"user.php?act=start_taocan&tid=" .$row['tid']. "\" onclick=\"if (!confirm('你确认要开始配送吗')) return false;\">开始配送</a>";
	   }
        $arr[] = array('tid'       => $row['tid'],
                       'goods_id'       => $row['goods_id'],
					   'order_id'       => $row['order_id'],
                       'add_time'     => local_date($GLOBALS['_CFG']['time_format'], $row['add_time']),
					   'start_time'     => local_date($GLOBALS['_CFG']['time_format'], $row['start_time']),
					   'end_time'     => local_date($GLOBALS['_CFG']['time_format'], $row['end_time']),
                       'taocan_name'   => $row['taocan_name'],
					   'taocan_weight'   => $row['taocan_weight']*2 .'斤',
					   'song_weight'   => $row['song_weight']*2 .'斤',
					   'zengsong'   => $row['zengsong']*2 .'斤',
					   'handler'   => $row['handler'],
					   'status'   => getStatus($row['status']),
					   'y_weight'   => ($row['taocan_weight']+$row['zengsong']-$row['song_weight'])*2 .'斤',
					   'taocan_price'   => $row['taocan_price']
                      );
    }
...
function get_order_taocan($tid)
{
	...
	if($res)
	{
		if($res['status']==2)
	   {
		   $res['handler'] = "<a href=\"user.php?act=stop_taocan&tid=" .$res['tid']. "\" onclick=\"if (!confirm('你确认要暂停配送吗？想要开始的时候记得来套餐管理里点击\"开始配送\"！')) return false;\">暂停配送</a>";
	   }
	    else if($res['status']==4)
	   {
		   $res['handler'] = " <a href=\"user.php?act=stop_taocan&tid=" .$res['tid']. "\" onclick=\"if (!confirm('你确认要开始配送吗')) return false;\">开始配送</a>";
	   }
	   
                       $res['add_time']= local_date($GLOBALS['_CFG']['time_format'], $res['add_time']);
					   $res['start_time' ]= local_date($GLOBALS['_CFG']['time_format'], $res['start_time']);
					   $res['end_time']     = local_date($GLOBALS['_CFG']['time_format'], $res['end_time']);
                       $res['taocan_name']   = $res['taocan_name'];
 ...
 * @return  boolean
 */
function get_peisong_date_list($tid,$status)
{
	if($status==2){
		$wheresql=" ps_time<".time();
	}
	else{
		$wheresql="order_taocan_id = '$tid' and status='$status'";
	}
    $sql = "SELECT ps_time FROM " .$GLOBALS['ecs']->table('peisong') . " WHERE $wheresql";
    $pstime = $GLOBALS['db']->getAll($sql);
	$res=array();
    foreach($pstime as $val)
	{
		$res[]=$val['ps_time'];
 ...
 * @return  boolean
 */
function get_ps_address($addressid)
{
    $sql = "SELECT shipping_area_name,address_id FROM " .$GLOBALS['ecs']->table('user_address') . " AS a LEFT JOIN " .$GLOBALS['ecs']->table('shipping_area') . "AS s ON a.ps_shiping= s.shipping_area_id WHERE a.address_id=$addressid ";
...
function get_taocan_psweight($goods_id){
	return $GLOBALS['db']->getone("SELECT ps_weight FROM ". $GLOBALS['ecs']->table('goods') ." WHERE goods_id=$goods_id") ;
}
/**
 *  获取用户可以和并的订单数组
 *
