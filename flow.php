...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    else
    {
        ...
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent))
        {
			...
			$result['num'] = stripslashes($goods->number);
        }
        else
        {
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	if($consignee['ps_shiping']){
	 $consignee['shipping_area_name']=get_area_name($consignee['ps_shiping']);
	}
    /* 检查收货人信息是否完整 */
...
elseif ($_REQUEST['step'] == 'ajax_cart'){
	 include_once('includes/cls_json.php');
	/* 取得商品列表，计算合计 */
    $cart_goods = get_cart_goods();
    $smarty->assign('goods_list_cart', $cart_goods['goods_list']);
    $smarty->assign('total', $cart_goods['total']);
	 $smarty->assign('action','ajax_cart');
	$price_show="共".$cart_goods['total']['real_goods_count']."件,总计".$cart_goods['total']['goods_amount']."元";
	 $json  = new JSON;
      $tpl = 'library/cart_info.lbi';

    make_json_result($smarty->fetch($tpl),$price_show);
}
else
{
    /* 标记购物流程为普通商品 */
