...
header('Pragma: no-cache');
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
	{
        ...
        $xml .= '<type>'.$cat_type_num.'</type>';
        $xml .= '<description><![CDATA['.$group_buy['small_desc'].']]></description>';
        ...
        $xml .= '</display><shops>';
        $xml .= '<shop>';
        $xml .= '<merchantName>'.$group_buy['suppliers_name'].'</merchantName>';
        $xml .= '<merchantPhone>'.$group_buy['phone'].'</merchantPhone>';
        $xml .= '<merchantAddr>'.$group_buy['address'].'</merchantAddr>';
