...
header('Pragma: no-cache');
	$sql = "SELECT a.group_id,a.group_name,a.group_image,a.start_time,a.group_need,a.cat_id,a.is_hdfk,a.already_orders,a.end_time,a.market_price,a.ext_info,a.city_id,a.group_desc,b.suppliers_name,b.phone,b.address " .
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
	{
		...
		$group_url = rewrite_groupurl('team.php',array('id' => $group_buy['group_id']));
	     ...
	     $sql = 'SELECT city_name,eng_name FROM ' . $GLOBALS['ecs']->table('group_city') . 
		  ...
		  $xml .= '<category>'.$cat_type_name.'</category>';
		  //$xml .= '<tag></tag>';
		 ...
		 $xml .= '<dueTs>' . $group_buy['past_time'] . '</dueTs>';
		 $xml .= '<tip></tip>';
         $xml .= '<merchants><merchant>';
