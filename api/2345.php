...
header('Pragma: no-cache');
	...
	$sql = "SELECT a.group_id,a.group_name,a.group_image,a.start_time,a.group_need,a.goods_type,a.group_type,a.cat_id,a.is_hdfk,a.already_orders,a.end_time,a.past_time,a.market_price,a.ext_info,a.group_desc ,a.goods_comment,a.city_id,b.suppliers_name,b.suppliers_site,b.west_way,b.east_way,b.phone,b.address " .
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
	{
		...
		$sql = 'SELECT city_name,eng_name FROM ' . $GLOBALS['ecs']->table('group_city') . 
		  ...
		  $city = $db->getRow($sql);
		  $group_buy['city_name'] = $city['city_name'];
		  $group_buy['eng_name'] = $city['eng_name'];
	    $sql = 'SELECT city_name,eng_name FROM ' . $GLOBALS['ecs']->table('group_city') . 
		       " WHERE city_id ='$city_id'";
		$city = $db->getRow($sql);
		...
		$group_url = rewrite_groupurl('team.php',array('id' => $group_buy['group_id']));
		 ...
		 $xml .= '<spend_start_time>'. $group_buy['start_time'] .'</spend_start_time>';
         $xml .= '<spend_close_time>'. $group_buy['end_time'] .'</spend_close_time>';
         $xml .= '<longitude>'.$group_buy['west_way'].'</longitude>';
         $xml .= '<latitude>'.$group_buy['east_way'].'</latitude>';
		 $xml .= '<collection>2000</collection>';
