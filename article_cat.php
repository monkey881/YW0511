...
if (!$smarty->is_cached('article_cat.dwt', $cache_id))
{
	...
	foreach(get_cat_articles($cat_id, $page, $size ,$keywords,$_GET['user_id']) as $article){
		$user=get_user_pic($article['user_id']);
		$article['userpic']=$user['userpic'];
		$article['user_name']=$user['user_name'];
		$row[]=$article;
	}
	$row2=array();
	
		foreach(get_cat_articles(21, $page, 2 ,$keywords) as $article){
			$user=get_user_pic($article['user_id']);
			$article['userpic']=$user['userpic'];
			$article['user_name']=$user['user_name'];
			$row2[]=$article;
    ...
    $smarty->assign('artciles_list',    $row);
	$smarty->assign('artciles_list2',    $row2);
...
if(($cat_id=='22' || $cat_id=='21' )&& $keywords==''&&$_GET['user_id']==''){
	$smarty->display('article_cat2.dwt', $cache_id);
}
else if(($keywords ||$_GET['user_id'])&&($cat_id=='22' || $cat_id=='21') ){
...
function get_user_pic($user_id){
	if($user_id){
	return $GLOBALS['db']->getRow('select userpic,user_name from '. $GLOBALS['ecs']->table('users') .' where user_id='.$user_id);
	}
	else
	{
		return;
