...
$image_a1 = new cls_image($_CFG['bgcolor']);
...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	$img_name2 = basename($image_a1->upload_image($_FILES['brand_logo2'],'brandlogo'));
     /*处理URL*/
    ...
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo,brand_logo2, is_show, sort_order,gend,age,tel,yangzhi_age,address,addtime,sologan,toptext,image1,image2,nongchanpin,bottomtext,type) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]', '$img_name','$img_name2', '$is_show', '$_POST[sort_order]','$_POST[gend]','$_POST[age]','$_POST[telephone]','$_POST[yangzhi_age]','$_POST[address]','$addtime','$_POST[sologan]','$_POST[toptext]','$image1','$image2','$_POST[nongchanpin]','$_POST[bottomtext]','$_POST[type]')";//二次开发
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_logo2, brand_desc, brand_logo, sologan,is_show, sort_order,age,gend,tel,address,yangzhi_age,image1,image2,toptext,nongchanpin,bottomtext,type ".//二次开发
...
elseif ($_REQUEST['act'] == 'remove')
{
	...
	$sql = "SELECT brand_logo2 FROM " .$ecs->table('brand'). " WHERE brand_id = '$id'";
    $logo_name = $db->getOne($sql);
    if (!empty($logo_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/brandlogo/' .$logo_name);
...
elseif ($_REQUEST['act'] == 'drop_logo2')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $brand_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    ...
    /* 取得logo名称 */
    $sql = "SELECT brand_logo2 FROM " .$ecs->table('brand'). " WHERE brand_id = '$brand_id'";
    $logo_name = $db->getOne($sql);

    if (!empty($logo_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/brandlogo/' .$logo_name);
        $sql = "UPDATE " .$ecs->table('brand'). " SET brand_logo2 = '' WHERE brand_id = '$brand_id'";
        $db->query($sql);
    }
    $link= array(array('text' => $_LANG['brand_edit_lnk'], 'href' => 'brand.php?act=edit&id=' . $brand_id), array('text' => $_LANG['brand_list_lnk'], 'href' => 'brand.php?act=list'));
    sys_msg($_LANG['drop_brand_logo_success'], 0, $link);
