...
require_once(ROOT_PATH . '/' . ADMIN_PATH . '/includes/lib_taocan.php');
/* act操作项的初始化 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 显示商品列表页面 */
    assign_query_info();
    $smarty->display('taocan_list.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 初始化变量 */
    $cat['title']       = !empty($_POST['taocan_name'])       ?$_POST['taocan_name']     : '';
    $cat['goods_id']    = !empty($_POST['goods_id'])    ? intval($_POST['goods_id'])  : 0;
    $cat['shxiaxian']   = !empty($_POST['shxiaxian'])   ? intval($_POST['shxiaxian']*500) : 0;

    /* 入库的操作 */
    if ($db->autoExecute($ecs->table('taocan'), $cat) !== false)
    {
        $cat_id = $db->insert_id();
       
        /*添加链接*/
        $link[0]['text'] = '添加可选商品';
        $link[0]['href'] = 'goods_taocan.php?act=addshop&tid='.$cat_id;
