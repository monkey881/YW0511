...
<div class="taocan_left">
        ...
        <div class="taocan_nums">
            ...
            <p>请选择蔬菜的领取点</p>
                ...
                <span>火车站明珠大厦</span><span>图书馆新都宾馆</span>
            </div>
        </div>
    </div>
...
.span_choiceable{
    ...
    <script>
         ...
         function return_result(data){
          data = eval("("+data+")");
          var html = "<p>可选的商品数是<span class=\"choiceable_num\">"+data.choiceable_num+"</span></p>"
            ...
            $$('.taocan_update').live("click",function(){
                ...
                $$('.nofixed .span_choiceable').each(function(){
                    if($$(this).hasClass("itemselected")){//压入被选中的ID
                        //判断数量
						if(Number($$(this).find(".goods_num input").val())%0.5!=0)
						{
							alert("请输入0.5倍数");
							return;
							}
						...
						{
                        num =Number(num) + Number($$(this).find(".goods_num input").val());
                        goods_id.push($$(this).children('.goods_hidden_id').text()+"#"+$$(this).find(".goods_num input").val());
						}
                    }
                });
                if(num !=$$('.choiceable_num').text()){
