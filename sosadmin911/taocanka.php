...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('taocanka_list.htm');
}
...
function get_snatchlist()
{
    ...
    if ($result === false)
    {
        ...
        $where = (!empty($filter['keywords'])) ? " AND user_id like '%". mysql_like_quote($filter['keywords']) ."%'" : '';

        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('taocan_ka') .
               " WHERE id>0 " . $where;
        ...
        /* 获活动数据 */
        $sql = "SELECT * ".
               " FROM " . $GLOBALS['ecs']->table('taocan_ka') .
               " WHERE id>0 "  . $where .
    ...
    {
        $row[$key]['sctime'] = local_date($GLOBALS['_CFG']['time_format'], $val['sctime']);
        $row[$key]['sytime']   = local_date($GLOBALS['_CFG']['time_format'], $val['sytime']);
       
    }

    $arr = array('taocanka' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
