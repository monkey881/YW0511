...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if (!empty($cat))
    {
		...
		$smarty->assign('jianshao',   $cat['jianshao']);
    }
    else
    {
        /* 如果分类不存在则返回首页 */
	...
	//最受欢迎
	$hysql="SELECT * FROM  ". $GLOBALS['ecs']->table('goods') ." AS g LEFT JOIN ". $GLOBALS['ecs']->table('goods_cat') ." AS c ON g.goods_id=c.goods_id WHERE g.cat_id=$cat_id AND c.cat_id=69 ORDER BY g.sort_order";
	$hy=$GLOBALS['db']->selectLimit($hysql, 3, ($page - 1) * $size);
	$num=1;
	$arr=array();
    ...
    $smarty->assign('cat_id',      $cat_id); //商品分类
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id,jianshao FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
