...
function reg(str){
  var bt=__$(str+"_b").getElementsByTagName("h2");
  for(var i=0;i<bt.length;i++){
    bt[i].subj=str;
    bt[i].pai=i;
    bt[i].style.cursor="pointer";
    bt[i].onmousemove=function(){
      __$(this.subj+"_v").innerHTML=__$(this.subj+"_h").getElementsByTagName("blockquote")[this.pai].innerHTML;
      for(var j=0;j<__$(this.subj+"_b").getElementsByTagName("h2").length;j++){
        var _bt=__$(this.subj+"_b").getElementsByTagName("h2")[j];
        var ison=j==this.pai;
        _bt.className=(ison?"":"h2bg");
      }
    }
  }
  __$(str+"_h").className="none";
  __$(str+"_v").innerHTML=__$(str+"_h").getElementsByTagName("blockquote")[0].innerHTML;
}
