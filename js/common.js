...
function addToCart1(goodsId, parentId) {
    ...
    if (document.getElementById("shopbox")) {
        ...
        document.getElementById("shopbox").style.top = (document.minitop - 150) + "px";
...
function addToCartResponse1(result) {
    if (result.error > 0) {
        ...
        if (result.one_step_buy == '1') {
            ...
            document.getElementById("shopcart").innerHTML = result.content;
...
function eyloadcart() {
    $.ajax({
        ...
        success: eyloadcartResponse,
        dataType: 'json',
        type: 'POST'
    });
}

function eyloadcartResponse(result) {
    if (result.error > 0) {
        ...
        $('#eycart').html(result.content);
        $('#car_good_num').html(result.message);
        vv('cat_ul');
    }
}
...
function newPrice(id) {
    $.ajax({
        url: 'snatch.php?act=new_price_list&id=' + id,
        data: {},
        success: newPriceResponse,
        dataType: 'text',
        type: 'GET'
    });
}
...
function pmwin(action, param) {
    ...
    if (action == 'open') {
        ...
        if (!_$('pmlayer')) {
            ...
            _$('pmlayer').innerHTML = '<div style="width: 800px; background: #666666; margin: 5px auto; text-align: left">' +
              '<div style="width: 800px; height: ' + pmheight + 'px; padding: 1px; background: #FFFFFF; border: 1px solid #7597B8; position: relative; left: -6px; top: -3px">' +
              '<div onmousedown="pmwindrag(event, 1)" onmousemove="pmwindrag(event, 2)" onmouseup="pmwindrag(event, 3)" style="cursor: move; position: relative; left: 0px; top: 0px; width: 800px; height: 30px; margin-bottom: -30px;"></div>' +
              '<a href="###" onclick="pmwin(\'close\')"><img style="position: absolute; right: 20px; top: 15px" src="images/close.gif" title="关闭" /></a>' +
              '<iframe id="pmframe" name="pmframe" style="width:' + pmwidth + 'px;height:100%" allowTransparency="true" frameborder="0"></iframe></div></div>';
        }
        _$('pmlayer').style.display = '';
        _$('pmlayer').style.top = ((clientHeight - pmheight) / 2 + scrollTop) + 'px';
        if (!param) {
...
function pmwindrag(e, op) {
    if (op == 1) {
        ...
        pmwindragstart[2] = parseInt(_$('pmlayer').style.left);
        pmwindragstart[3] = parseInt(_$('pmlayer').style.top);
        doane(e);
    }
    else if (op == 2 && pmwindragstart[0]) {
        var pmwindragnow = is_ie ? [event.clientX, event.clientY] : [e.clientX, e.clientY];
        _$('pmlayer').style.left = (pmwindragstart[2] + pmwindragnow[0] - pmwindragstart[0]) + 'px';
        _$('pmlayer').style.top = (pmwindragstart[3] + pmwindragnow[1] - pmwindragstart[1]) + 'px';
        doane(e);
    }
    else if (op == 3) {
        pmwindragstart = [];
