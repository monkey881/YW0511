...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
	...
	$db->query("UPDATE " .$ecs->table('article'). " SET hot_num=hot_num+1 WHERE article_id = ".$article['article_id']);
    if (empty($article))
    {
        ecs_header("Location: ./\n");
        exit;
    ...
    if ($row !== false)
    {
        ...
        if (empty($row['author']) || $row['author'] == '_SHOPHELP')
        {
            if($row['user_id']){
            	...
            	$row['author']=$GLOBALS['db']->getone("select user_name from ". $GLOBALS['ecs']->table('users') ." where user_id=".$row['user_id']);
			}
			else{
