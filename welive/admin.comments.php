...
if($action == 'default'){
	...
	选择:&nbsp;<select name="u"><option value="0">全部客服</option>'. $useroptions .'</select>&nbsp;&nbsp;<select name="days"><option value="0">全部留言</option>&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" name="delete" onclick="return confirm(\'确定删除所选留言吗?\');" value=" 快速删除 " />
	</form>
	</td>
	</tr>
	</table>
	...
	if($maxrows['value'] < 1){
		...
		while($comment = $DB->fetch($getcomments)){
			...
			<td width="35%">'.nl2br($comment['content']). '</a></td>
