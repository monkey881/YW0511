...
require_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/admin/article.php');
...
if ($action == 'default')
{
	...
	$cal = new Calendar();
	$ps_dates=$cal->getPeisongDate(3);
	$arr_dps=array();
	foreach($taocan as $items){
		foreach($ps_dates as $dates){
			$psweight=$items['ps_weigth'];
			
			$tid=$items['tid'];
			if(!$GLOBALS['db']->getone("SELECT COUNT(*) FROM ". $GLOBALS['ecs']->table('peisong') ." WHERE weight<'$psweight' AND ps_time=$dates AND order_taocan_id=$tid")){
				$dps['tid']=$tid;
				$dps['taocan_name']=$items['taocan_name'];
				$dps['date']=date('m-d',$dates);
				$arr_dps[]=$dps;
				}
		}
	}
...
elseif ($action == 'act_edit_profile')
{
	...
	$other['userpic'] = $userpic = isset($_POST['userpic']) ? trim($_POST['userpic']) : '';
    $sel_question = empty($_POST['sel_question']) ? '' : compile_str($_POST['sel_question']);
    $passwd_answer = isset($_POST['passwd_answer']) ? compile_str(trim($_POST['passwd_answer'])) : '';
	
	$user_img   = $image->upload_image($_FILES['userpic_file']); 
	if($user_img){
	$other['userpic']=$user_img;
	}
    /* 更新用户扩展字段的数据 */
...
elseif ($action == 'order_list')
{
    ...
    $orders = get_user_orders($user_id, $pager['size'], $pager['start'],$_REQUEST['status']);
...
/* 查看订单列表 */
elseif ($action == 'taocan')
{
 	include_once(ROOT_PATH . 'includes/lib_transaction.php');
	include_once(ROOT_PATH . 'includes/calendar.class.php');
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 0;

    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_taocan'). " WHERE status>1 AND status<4 AND user_id = '$user_id'");

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);

    $taocans = get_user_orders_taocan($user_id,$pager['size'], $pager['start']);
	$smarty->assign('user_id',  $user_id);
    $smarty->assign('merge',  $merge);
    $smarty->assign('pager',  $pager);
    $smarty->assign('taocans', $taocans);
...
elseif ($action == 'rili')
{
	...
	$songed=get_peisong_date_list($taocan['tid'],2);
...
elseif ($action == 'stop_taocan')
{
	...
	$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_taocan'), $data, 'UPDATE', "tid = $tid");
     ecs_header("Location: user.php?act=taocan\n");
       exit;
...
elseif ($action == 'start_taocan')
{
	...
	$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_taocan'), $data, 'UPDATE', "tid = $tid");
	$sql="select ps_id from " .$ecs->table('peisong'). " where order_taocan_id=$tid and status=0 order by ps_time asc";
	$peisong=$db->getAll($sql);
	$peisong_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('peisong'). " WHERE order_taocan_id=$tid and status=0");
	$cal = new Calendar();
	$psdate=$cal->getPeisongDate($peisong_count);
	$i=0;
	foreach($peisong as  $item){
		$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('peisong'),array('ps_time'=>$psdate[$i]), 'UPDATE', "ps_id = $item[ps_id]");
		$i++;
		}
...
elseif ($action == 'act_edit_address')
{
    ...
    $address = array(
		...
		'ps_shiping'       => isset($_POST['ps_shiping'])      ? intval($_POST['ps_shiping'])     : 0,
		'defaultps'       => isset($_POST['defaultps'])      ? intval($_POST['defaultps'])     : 0,
...
//分享美食开始
else if ($action == 'good_food')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
	require_once(ROOT_PATH . "includes/fckeditor/fckeditor.php");

	$sql="SELECT * FROM ". $ecs->table('article') ." WHERE user_id=$user_id ";
	$article=$db->getAll($sql);
	$article_list=array();
	foreach($article as $art){
		$article2['title']=$art['title'];
		$article2['add_time']=date('y-m-d',$art['add_time']);
		$article2['handler']="<a href='user.php?act=good_food_edit&id=".$art['article_id']."'>编辑</a>";
		$article_list[]=$article2;
	}
    ...
    $smarty->display('user_article.dwt');
}
//分享美食开始
else if ($action == 'good_food_add')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
	require_once(ROOT_PATH . "includes/fckeditor/fckeditor.php");
    ...
    $smarty->assign('ur_here',     $_LANG['article_add']);
    $smarty->assign('action_link', array('text' => $_LANG['03_article_list'], 'href' => 'article.php?act=list'));
    $smarty->assign('form_action', 'insert_article');
...
else if ($action == 'good_food_edit')
{
    ...
    $smarty->assign('ur_here',     $_LANG['article_add']);
    $smarty->assign('action_link', array('text' => $_LANG['03_article_list'], 'href' => 'article.php?act=list'));
    $smarty->assign('form_action', 'update_article');
...
else if ($action == 'insert_article')
{
    ...
    if ((isset($_FILES['file']['error']) && $_FILES['file']['error'] == 0) || (!isset($_FILES['file']['error']) && isset($_FILES['file']['tmp_name']) && $_FILES['file']['tmp_name'] != 'none'))
    {
        ...
        {
            show_message($_LANG['invalid_file']);
        }
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open, author, ".
                "author_email, keywords, content, add_time, file_url, open_type, link, description,user_id) ".
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$article_type', 'is_open', ".
                "'$_POST[author]', '', '', '$_POST[FCKeditor1]', ".
                "'$add_time', '$file_url', '$open_type', '', '','$user_id')";
    $db->query($sql);

   

    $link[0]['text'] = $_LANG['continue_add'];
    $link[0]['href'] = 'user.php?act=good_food';



    show_message($_LANG['articleadd_succeed'],0, 'user.php?act=good_food');
}
if ($_REQUEST['act'] =='update_article')
{
    ...
    if ($db->query("UPDATE ". $ecs->table('article')." SET title='$_POST[title]', cat_id='$_POST[article_cat]',  author='$_POST[author]', author_email='$_POST[author_email]',  file_url ='$file_url',  content='$_POST[FCKeditor1]' where article_id=".$_POST['id']))
    {
